...
- **語言登錄表是單一事實來源**：多語化後 script profile、翻譯 prompt 名、whisper code、術語 carrier 句、empty_state 全部集中在 `shared/languages.json` 一份，Rust `include_str!`、TS `import`、Python（`checks.py` / `run_eval.py`）各自讀同一份，避免三端各自寫死漂移。加語言＝補一筆，見 `docs/ADD_LANGUAGE.md`。
