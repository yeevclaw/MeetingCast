...
/// Tri-state API-key validation used by the Welcome wizard. Both endpoints
/// are free metadata GETs — no tokens are billed. The result is a string
...
#[tauri::command]
pub async fn validate_deepgram_key(key: String) -> Result<String, String> {
    let resp = client()?
