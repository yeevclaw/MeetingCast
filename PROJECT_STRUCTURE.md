...
│   │   └── TranslationWindow.tsx   # 譯文視窗（en/vi 共用，由 lang prop 區分）
│   ├── components/
...
### `src/`
單一 React bundle，三個視窗共用。`main.tsx` 用 `getCurrentWindow().label`（`control` / `en` / `vi`）決定 render 哪個視窗 component。Tailwind v4，所有色彩走 `App.css` 的 `@theme paper-*` token（warm paper / 墨色，狀態色都在暖色家族內）。
...
[Sidecar] --stdio JSON--> [Rust sidecar.rs]
              ...
              ┌───────────────┼────────────────┐
              ▼               ▼                ▼
         [Control]        [English]       [Vietnamese]
              │
              │ control 收到 is_final → invoke("translate", id, text, target)
              ▼
        [Rust translator.rs] --SSE--> Anthropic API
              │
              ▼ emit("translation:chunk:en") / ("translation:chunk:vi")
         [English Window]   [Vietnamese Window]
```
